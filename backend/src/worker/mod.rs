...
async fn test_single_video(
    ...
    let result = VideoResult {
        ...
        trigger_method: video_result.trigger_method.clone(),
        stutter_count: video_result.stutter_count,
        stutter_duration_ms: video_result.stutter_duration_ms,
        play_duration_sec: video_result.play_duration_sec,
        stutter_ratio: video_result.stutter_ratio,
...
async fn save_video_result(db: &SqlitePool, result: &VideoResult) -> anyhow::Result<()> {
    ...
    .bind(&result.error_msg)
    .bind(result.test_count)
    .bind(&result.trigger_method)
    .bind(result.stutter_count)
    .bind(result.stutter_duration_ms)
    .bind(result.play_duration_sec)
    .bind(result.stutter_ratio)
    .bind(result.video_width)
    .bind(result.video_height)
    .bind(result.video_duration_sec)
    .bind(&result.created_at)
    .execute(db)
    .await?;
