...
<style scoped>
.create-task :deep(.n-card) { background:var(--panel-gradient); }
.tpl-card { background:var(--panel-gradient) !important; }
@media (max-width:760px) { .create-task { max-width:none; }.view-intro { margin-bottom:18px; }.create-task :deep(.n-grid) { display:block !important; }.create-task :deep(.n-gi) { margin-bottom:14px; }.create-task :deep(.n-card) { padding:0; }.tpl-row { align-items:flex-start; flex-direction:column; }.tpl-row .n-space { width:100%; }.tpl-row .n-button { flex:1; }.create-task :deep(.n-radio-group) { display:grid; grid-template-columns:1fr 1fr; }.create-task :deep(.n-radio) { margin:0; }.create-task :deep(.n-upload-dragger) { padding:10px; } }
@media (max-width:430px) { .create-task :deep(.n-radio-group) { grid-template-columns:1fr; } }
</style>
