...
<style scoped>
.runs-page { max-width:1200px; }.page-title { color:var(--text-primary); font-family:var(--font-display); font-weight:500; letter-spacing:-.05em; }.page-subtitle { color:var(--text-secondary); }.back-btn,.btn,.date-input,.select { border-color:var(--border-color); border-radius:5px; background:var(--bg-card); color:var(--text-secondary); font-family:var(--font-mono); font-size:10px; }.back-btn:hover,.btn:hover { border-color:var(--border-color-hover); background:var(--bg-hover); }.btn.primary { border-color:var(--color-primary); background:var(--color-primary); color:#06201f; }.btn.primary:hover { background:var(--color-primary-active); }.btn.danger { border-color:var(--color-danger); color:var(--color-danger); }.btn.danger:hover { background:var(--color-danger); color:#27120d; }.filter-bar { border-color:var(--border-color); border-radius:9px; background:rgba(15,32,50,.55); }.run-card { position:relative; overflow:hidden; border-color:var(--border-color); border-radius:10px; background:linear-gradient(145deg,var(--bg-card),rgba(15,32,50,.76)); box-shadow:var(--shadow-card); }.run-card::before { position:absolute; top:0; left:0; width:2px; height:100%; background:var(--color-primary); opacity:.7; content:''; }.run-time,.task-id,.detail-value { font-family:var(--font-mono); }.run-body,.task-links,.run-export { border-top-color:var(--border-color); }.task-id { border:1px solid var(--border-color); background:var(--bg-alt); color:var(--color-primary); }.status-completed { background:var(--color-primary-bg); color:var(--color-primary); }.status-running { background:rgba(136,169,255,.12); color:var(--color-info); }.status-failed { background:rgba(255,138,101,.12); color:var(--color-danger); }.status-pending,.status-cancelled { background:rgba(155,176,194,.1); color:var(--text-secondary); }.empty-state { border:1px dashed var(--border-color); border-radius:10px; }.empty-state h3 { font-family:var(--font-display); }
@media (max-width:650px) { .page-header { gap:15px; flex-direction:column; }.header-actions { width:100%; }.header-actions .btn { flex:1; }.filter-bar { align-items:stretch; }.spacer { display:none; }.date-input,.select { flex:1; }.run-body { flex-wrap:wrap; gap:12px; }.task-link-row { flex-wrap:wrap; } }
</style>

<style scoped>
.filter-bar { background:var(--panel-soft); }
.run-card { background:var(--panel-gradient); }
@media (max-width:760px) { .runs-page { max-width:none; }.page-header { align-items:stretch; }.filter-bar { display:grid; grid-template-columns:auto 1fr; gap:8px; }.filter-bar .spacer { display:none; }.filter-bar .btn,.filter-bar .auto-refresh { grid-column:1 / -1; justify-content:center; }.runs-list { gap:10px; }.run-card { padding:15px; }.run-header { align-items:flex-start; gap:10px; flex-direction:column; }.run-status-row { width:100%; flex-wrap:wrap; }.task-link-row { align-items:flex-start; }.run-export { flex-wrap:wrap; }.run-export .btn { flex:1; } }
</style>
